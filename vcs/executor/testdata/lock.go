...
func main() {
	...
	if *mode != "rlock" && *mode != "lock" {
		log.Fatal(*prefix, "mode must be rlock or lock")
	}
	...
	select {
	case <-lockAcquired:
		log.Println(*prefix, *mode, "acquired")
		<-time.After(*hold)
		if *mode == "lock" {
			lock.Unlock()
		} else {
		...
		log.Println(*prefix, *mode, "released")
	case <-time.After(*timeout):
		log.Fatal(*prefix, " timeout ", *mode)
	}
}
