...
import {Ref} from 'vue'
import {PullCodebaseInput} from '../__generated__/types'
import {DeepMaybeRef} from '@vueuse/core'
import {PullCodebaseMutation, PullCodebaseMutationVariables} from "./__generated__/usePullCodebase";
...
export const pullCodebaseUpdateResolver: UpdateResolver<PullCodebaseMutation,
    PullCodebaseMutationVariables> = (result, args, cache, info) => {
    if (!result) {
        return
    }
    if (result.pullCodebase && result.pullCodebase.__typename) {
        const key = cache.keyOfEntity({__typename: result.pullCodebase.__typename, id: result.pullCodebase.id})
        cache.inspectFields(key).filter(({fieldName}) => fieldName === 'changes').forEach((f) => {
