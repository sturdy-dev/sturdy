<template>
  <banner v-if="!gitHubAppValidation.ok" status="error">
    <label
      >The GitHub app is not correctly configured. Check the GitHub app permissions & events.
      <ul v-if="gitHubAppValidation.missingPermissions.length > 0" class="list-disc pl-4">
        <li>
          The app is missing the following permissions:
          <ul class="list-disc pl-4">
            <li v-for="permission in gitHubAppValidation.missingPermissions" :key="permission">
              {{ permission }}
            </li>
          </ul>
        </li>
      ...
      <ul v-if="gitHubAppValidation.missingEvents.length > 0" class="list-disc pl-4">
        <li>
          The app is missing the following events:
          <ul class="list-disc pl-4">
