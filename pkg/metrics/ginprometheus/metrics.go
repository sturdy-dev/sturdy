...
func NewPrometheus(subsystem string, logger *zap.Logger, customMetricsList ...[]*Metric) *Prometheus {
...
func (p *Prometheus) registerMetrics(subsystem string) {
	...
	for _, metricDef := range p.MetricsList {
		...
		if err := prometheus.Register(metric); err != nil {
			p.log.Error("metric could not be registered in Prometheus", zap.String("name", metricDef.Name), zap.Error(err))
		}
		switch metricDef {
		case reqCnt:
