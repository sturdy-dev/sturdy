...
func (r *memberRepository) GetByUserIDAndOrganizationID(ctx context.Context, userID, organizationID string) (*organization.Member, error) {
	...
	if err := r.db.GetContext(ctx, &mem, `SELECT id, user_id, organization_id, created_at, deleted_at FROM organization_members WHERE user_id = $1 AND organization_id = $2`, userID, organizationID); err != nil {
...
func (r *memberRepository) ListByOrganizationID(ctx context.Context, id string) ([]*organization.Member, error) {
	...
	if err := r.db.SelectContext(ctx, &res, `SELECT id, user_id, organization_id, created_at, deleted_at FROM organization_members WHERE organization_id = $1`, id); err != nil {
...
func (r *memberRepository) ListByUserID(ctx context.Context, id string) ([]*organization.Member, error) {
	var res []*organization.Member
	if err := r.db.SelectContext(ctx, &res, `SELECT id, user_id, organization_id, created_at, deleted_at FROM organization_members WHERE user_id = $1`, id); err != nil {
...
func (r *memberRepository) Create(ctx context.Context, mem organization.Member) error {
	if _, err := r.db.NamedExecContext(ctx, `INSERT INTO organization_members (id, user_id, organization_id, created_at, deleted_at)
		VALUES (:id, :user_id, :organization_id, :created_at, :deleted_at)`,
		mem); err != nil {
